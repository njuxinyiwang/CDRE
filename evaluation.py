...
def to_official_new(preds, features, current_relations, seen_relations):
    h_idx, t_idx, title = [], [], []
    ...
    for i in range(preds.shape[0]):
        ...
        for p in pred:
            if p != 0 and seen_relations[p] in current_relations:
                res.append(
                    {
                        'title': title[i],
                        'h_idx': h_idx[i],
...
def official_evaluate_new(tmp, features, current_relations, seen_relations):
    ...
    for x in features:
        title = x['title']
        titleset.add(title)
        title2vectexSet[title] = 1
        for i in range(len(x['labels'])):
            h_idx = x['hts'][i][0]
            t_idx = x['hts'][i][1]
            labels = x['labels'][i]
            for r in range(len(labels)):
                if labels[r] == 1 and r != 0 and seen_relations[r] in current_relations:
                    std[(title, r, h_idx, t_idx)] = 1
