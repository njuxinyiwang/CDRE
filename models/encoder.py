...
class DocREModel(nn.Module):
    ...
    def __init__(self, args, config, model, tokenizer,
        ...
        self.rel_head_extractor = nn.Linear(self.hidden_size * 2, emb_size)
        self.rel_tail_extractor = nn.Linear(self.hidden_size * 2, emb_size)
        self.head_extractor = nn.Linear(self.hidden_size * 2, emb_size)
        self.tail_extractor = nn.Linear(self.hidden_size * 2, emb_size)
        self.relation_to_context_attention = nn.MultiheadAttention(self.hidden_size, 1, batch_first=True)
    ...
    def get_rel_features(self, sequence_output, attention, attention_mask, entity_pos, hts, offset,  relation_cls):
        n, h, _, c = attention.size()
        ht_atts = []
        rss_rels = []
        for i in range(len(entity_pos)):  # for each batch
            ...
            # ht_att = ht_att / (ht_att.sum(1, keepdim=True) + 1e-30)
            ht_atts.append(ht_att)
            ht_att=F.normalize(ht_att, p=1, dim=1)
            rs_rels = []
            for k, rel in enumerate(relation_cls):
                _, rel_att = self.relation_to_context_attention(rel.expand(1, 1, -1), sequence_output[i].unsqueeze(0),
                                                             sequence_output[i].unsqueeze(0), key_padding_mask=~(
                                attention_mask[i].unsqueeze(0).bool()))
                rel_att = torch.squeeze(rel_att, 1).detach()

                r_att = ht_att * rel_att
                r_att = ht_att + r_att
                r_att = F.normalize(r_att, p=1, dim=1)
                rs_rel = contract("ld,rl->rd", sequence_output[i], r_att)
                hs_rel = torch.tanh(self.rel_head_extractor(torch.cat([hs, rs_rel], dim=-1)))
                ts_rel = torch.tanh(self.rel_head_extractor(torch.cat([ts, rs_rel], dim=-1)))
                rs_rels.append(torch.cat([hs_rel,ts_rel], dim=-1))
            rs_rels[0] = torch.cat([hs, ts], dim=-1)

            rs_rels = torch.transpose(torch.stack(rs_rels,dim=0),0,1)

            rss_rels.append(rs_rels)

        rss_rels = torch.cat(rss_rels,dim=0)
    ...
    def forward(self,
        ...
        if relation_cls is not None:
            rss_rels= self.get_rel_features(sequence_output,attention,attention_mask,entity_pos,hts, offset, relation_cls)
        else:
            rss_rels = None
        output["doc_attn"] = doc_attn
        if self.use_graph:
